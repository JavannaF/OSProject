...
void connection_handler(int socket_desc) {
     ...
     *
     * Note that on a socket we would get a 0 return value only when
     * the connection is closed. It is thus important to design good
     * protocols when we have to transfer messages of unknown size.
     *
     * For now we don't deal with partially received replies.
     */
    ...
    /* send() with flags = 0 is equivalent to write() on sockets.
     *
